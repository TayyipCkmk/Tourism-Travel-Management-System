...
<header class="header p-3 text-white">
    ...
    border-radius: 30px;">
            ...
            <a href="/user/mytickets?userid=${user.id}" class="p-3 btn me-2"style="margin-inline-start: 650px;background-color: #f7c86f;border-color: #f7c86f;border-radius: 30px;">My Tours</a>
...
<section class="home-content ">
    <div class="content">
        <div class="text-center container">
            <h1 class="title"style="color: white">Welcome ${user.username}</h1>
        </div>
        <div class="container">
            <div>
                <div class="list-group">
                    <c:forEach items="${flights}" var="item">
                        <li href="#" class="list-group-item mt-2 list-group-item-action text-white" aria-current="true">
                            <h4><img src="${item.fromLocation}" style="width: 150px"> - ${item.toFrom} - ${item.price} $ - ${item.date}</h4>
                            <div class="text-end">
                                <a href="/ticket/reservation?flightid=${item.id}&userid=${user.id}"
                                   class="p-3 me-2 btn btn-warning"style="color: #000;background-color: #f7c86f;border-color: #f7c86f;border-radius: 30px;">Buy Tour</a>
                            </div>
                        </li>
                    </c:forEach>
                </div>
